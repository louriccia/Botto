            ...
            if (args[1] == 'admin') {
                ...
                if (Object.values(livematch.commentators).includes(interaction.member.user.id) || (interaction.guild_id == '441839750555369474' && (Member.roles.cache.some(r => r.id == '862810190072381471')) && !Object.values(livematch.players).includes(interaction.member.user.id))) {
                    ...
                    if (status == 'first') {
                        ...
                        updateMessage(Object.values(livematch.players).filter(player => !livematch.races[current_race-1].ready[player]).map(player => "<@" + player + ">").join(" ") + " " + Object.values(livematch.commentators).map(comm => "<@" + comm + ">").join(" "), type, [raceEmbed(race)], raceComponents(race))
