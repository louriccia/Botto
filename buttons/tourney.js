    ...
    execute(client, interaction, args) {
        ...
        if (args[0] == "ranks") {
            ...
            if (args.includes("initial")) {
                ...
                for (var i = 0; i < interaction.message.components[0].components[0].options.length; i++) { //tourney
                ...
                for (var i = 0; i < interaction.message.components[2].components[0].options.length; i++) { //match
            ...
            //get tourneys
            var tourney_options = []
            var tourney_keys = Object.keys(tourney_tournaments_data)
            tourney_keys.forEach(key => {
                var t = tourney_tournaments_data[key]
                var t_stuff = {
                    oldest: null,
                    newest: null,
                    players: [],
            ...
            if (Object.values(tourney_scheduled_data).length > 0) {
                Object.values(tourney_scheduled_data).filter(match => match.datetime >= Date.now()).map(match => {
                    tourneyReport
                        ...
                        .addField(":crossed_swords: " + match.players.map(player => tourney_participants_data[player].name).join(" vs "), ":microphone2: " + (Object.values(match.commentary).length > 0 ? Object.values(match.commentary).filter(player => player !== "").map(player => console.log(player), tourney_participants_data[player].name).join(", "): "Sign up for commentary!"), true)
                        .addField('\u200B', '\u200B', true)
                })
            } else {
            ...
            function permabanComponents(permaban) {
                ...
                let selectoptions = []
                if (pban.type == "track") {
                    let permatrackbans = Object.values(livematch.races[1].events).filter(event => event.event == "permaban").filter(event => event.type == "track").map(event => event.selection)
                    for (let i = 0; i < 25; i++) {
