...
check('rule 3 · freed cubes come back beside the cube taking the turn', () => {
    const res = engine.resolveLine([
        at(CRAWLER, null, [{ id: 'greed' }, { id: 'wild' }]),
        at('symbiont', 'raze'),
        plain(),
    ], 'blue', [], {});
    ...
    assert.ok(ids.includes('greed') && ids.includes('wild'), 'both prisoners are on the table');
...
check('rule 3 · a wipeout spills its own hold', () => {
    const res = engine.resolveLine([
        plain(),
        at(CRAWLER, 'broken', [{ id: 'greed' }, { id: 'wild' }]),
        plain(),
    ], 'blue', [], {});
    assert.deepStrictEqual(res.broken, [CRAWLER]);
    const ids = res.set.map(s => s.id);
    assert.ok(ids.includes('greed') && ids.includes('wild'), 'both walked');
...
check('rule 4 · a jailer paroles one per turn, in order', () => {
    // Three prisoners that cannot destroy anything on their way out — Shmi, Anakin and the
    // Midi-chlorian all roll sides and a wipeout, and a wipeout takes only itself. A released Greed
    // or Wild carries a mine, and a mine going off would take the die with it, which is a different
    // rule being tested by accident.
    let cell = engine.slotOf({ id: DIE, held: [{ id: 'shmi' }, { id: 'anakin' }, { id: 'multiplier' }] });
    const out = [];
    ...
    assert.deepStrictEqual(out, ['shmi', 'anakin', 'multiplier'], 'first in, first out');
