...
module.exports = {
    ...
    execute({ interaction, database, db, member_id, member_name, member_avatar, user_key, user_profile } = {}) {
        interaction.client.buttons.get("challenge").execute({ client: interaction.client, interaction, args: ["random", interaction.options.getSubcommand()], database, db, member_id, member_name, member_avatar, user_key, user_profile })
    }
}