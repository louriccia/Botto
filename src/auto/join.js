...
exports.join_message = function (client, guildMember) {
    if (guildMember.guild.id == swe1r_guild && !testing) {
        ...
        random_join_message = random_join_message.replaceAll("replaceme", "<@" + guildMember.user + ">")

        //assign role
        client.channels.cache.get("441839751235108875").send({ content: random_join_message, flags: [4096] });
