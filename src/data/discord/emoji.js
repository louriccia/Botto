

exports.WhyNobodyBuy = "<:WhyNobodyBuy:589481340957753363>"
exports.ChanceCube = "<a:chance_cube:1235055236138270760>"

exports.emoji_id = function (string) {
    return string.split(":")[2].replace(">", "")
}

exports.emojimap = {
    BallQuadinaros: "<a:BallQuadinaros:964303313904484362>",
    Beedo: "<:Beedo:671544208699424778>",
    BenFret: "<:BenFret:1066223127891288084>",
    BenHi: "<a:BenHi:672640761501843466>",
    BenRage: "<a:BenRage:672640776718647367>",
    BolesRoor: "<:BolesRoor:525755286179545103>",
    BossLOL: "<:BossLOL:965847236107993088>",
    BrentRambo: "<a:BrentRambo:733158172373549067>",
    BullseyeSTEER: "<a:BullseyeSTEER:853758198049800212>",
    Bumpy: "<:Bumpy:672186652432662549>",
    BumpyThumb: "<:BumpyThumb:703107780860575875>",
    Creepio: "<:Creepio:1024170212666376253>",
    CryGon: "<:CryGon:1028180139995045968>",
    DieGon: "<:DieGon:1058560460586237982>",
    Dug: "<:Dug:525755299647193088>",
    DyeGon: "<a:DyeGon:1235055298281209886>",
    EyeYunga: "<:EyeYunga:1024168110938411050>",
    FlyGon: "<:FlyGon:1005209412899717211>",
    Fodesinbeed: "<a:Fodesinbeed:672640805055496192>",
    FudgeSang: "<:FudgeSang:1265474461373562983>",
    GasganoHands: "<:GasganoHands:867809385102704650>",
    GlassBones: "<:GlassBones:590941053272457236>",
    HammerHead: "<a:HammerHead:965839199452430358>",
    HighGon: "<:HighGon:1058649740516008006>",
    Howler: "<:Howler:1108624261582704691>",
    Jabba: "<:Jabba:794051891423215636>",
    JabbaSleep: "<:JabbaSleep:850463205611667487>",
    JarJarBlinks: "<a:JarJarBlinks:966191699586056222>",
    Kringester: "<:Kringester:1058636959708352522>",
    MarsReeee: "<:MarsReeee:728097837178683422>",
    MarsWut: "<:MarsWut:728097761152466947>",
    MonkaBinks: "<:MonkaBinks:850475959348887603>",
    NiktoBoo: "<a:NiktoBoo:964362648214986802>",
    Nugtosh: "<:Nugtosh:850055582782193664>",
    OdyOhNo: "<a:OdyOhNo:712555049816948767>",
    PadmeWhat: "<a:PadmeWhat:964049906895581194>",
    PadmeWut: "<:PadmeWut:1058642347879059558>",
    PitDroid: "<a:PitDroid:965844766971560016>",
    PodChamp: "<:PodChamp:732790510955266058>",
    PraiseMaja: "<:PraiseMaja:1065889750445199471>",
    RIPratts: "<a:RIPratts:674357178949304379>",
    Sebulba: "<:Sebulba:525755336163065866>",
    ShyGon: "<:ShyGon:1060076263341035520>",
    SlideFlip: "<a:SlideFlip:965874632253112330>",
    SlyGon: "<:SlyGon:964064233774006282>",
    SmugBen: "<:SmugBen:525755381155233797>",
    SmugBozzie: "<:SmugBozzie:451467451658403860>",
    SpyGon: "<:SpyGon:1058556680146784327>",
    Stitch: "<:Stitch:525755422217338900>",
    ThumbsUpBinks: "<a:ThumbsUpBinks:964280524141101086>",
    TriffianBow: "<a:TriffianBow:966052969432907866>",
    TuskenRaider: "<a:TuskenRaider:1106770539789434941>",
    WaldDance: "<a:WaldDance:964282241968660511>",
    WanSandwich: "<:WanSandwich:525755434628415488>",
    WattoLOL: "<a:WattoLOL:672640878736703509>",
    Whatto: "<a:Whatto:672640891139260467>",
    WhyGon: "<:WhyGon:807165170094702622>",
    WhyNobodyBuy: "<:WhyNobodyBuy:589481340957753363>",
    XamsterDance: "<a:XamsterDance:965870334089764864>",
    Yippee: "<:Yippee:850062819168354304>",
    YoungVader: "<:YoungVader:1024169142535852163>",
    aldar: "<:aldar:671598952923136011>",
    anakin: "<:anakin:671598858983178251>",
    andotent: "<:andotent:1023110359407657090>",
    ani_engine: "<:ani_engine:672639989019967490>",
    benspin: "<a:benspin:1028168991337885776>",
    // Two emoji share the name "binder" in the guild; the static one is keyed separately
    // because an object literal can only hold one `binder` and the animated one is the
    // Binder Cube's face.
    airbrake: "<:binder:525755271084113940>",
    binder: "<a:binder:672640399113715734>",
    botto: "<:botto:850138537440313344>",
    countdown: "<a:countdown:672640791369482251>",
    flamejet: "<a:flamejet:891129011550445579>",
    guidearrow: "<a:guidearrow:891128437354401842>",
    newrecord: "<a:newrecord:672640831882133524>",
    oovodoor: "<a:oovodoor:964369275559223306>",
    p1: "<:p1:671601240228233216>",
    p2: "<:p2:671601321257992204>",
    p3: "<:p3:671601364794605570>",
    pod: "<:pod:525755322355417127>",
    restart: "<a:restart:855623455751143434>",
    sandcrawler: "<a:sandcrawler:1106792071202865294>",
    seb_engine_l: "<:seb_engine_l:672640132163043353>",
    seb_engine_r: "<:seb_engine_r:672640206431584265>",
    sparks: "<a:sparks:672640526444527647>",
    speedo: "<a:speedo:672640865130381312>",
    swr: "<:swr:671547869118988328>",
    wipeout: "<a:wipeout:672640904473083904>",
    HappyFud: "<:HappyFud:1235054612327698512>",
    // Three thirds of one wide Ben. WideBen2 is his middle and is the Symbiont's raze face; the
    // other two are what the cubes he destroys are replaced with, so the three positions read as a
    // single picture lying across the line.
    WideBen1: "<:WideBen1:1235054858080616498>",
    WideBen2: "<:WideBen2:1235054897213476955>",
    WideBen3: "<:WideBen3:1235054932529385522>",
    SighGon: "<:SighGon:1235054800312209550>",
    server_icon: "<:server_icon:1235054663368179712>",
    SWEP1RCREXE: "<:SWEP1RCREXE:1235054770142711978>"
}

exports.console_emojis = {
    pc: "<:pc:1135825260726452265>",
    switch: "<:switch:1135825256527958076>",
    dc: "<:dc:1135825257970806905>",
    ps4: "<:ps4:1135825253268996167>",
    n64: "<:n64:1135825259895984128>",
    xbox: "<:xbox:1135825254753767505>"
}

exports.emoji = {
    goal: ["💎", "🥇", "🥈", "🥉", "<:BumpyThumb:703107780860575875>"],
    trugut: "📀",
    submit: "⏱️",
    reroll: "🔄",
    bribe: "💰",
    predict: "🔮",
    first: "❄️",
    bounty: "🎯",
    completed: "✅",
    sponsor: "📢",
    play: "🎲",
    settings: "⚙️",
    abandoned: "💨",
    achievement: "🏆",
    multiplayer: "🏁",
    feedback_positive: "👍",
    feedback_negative: "👎",
    menu: "<:menu:862620287735955487>",
    profile: "👤",
    help: "❔",
    hint: "💡",
    details: "🏷️",
    conditions: {
        mirrored: "",
        backwards: "",
        no_upgrades: "",
        skips: "",
        laps: ""
    }
}

exports.goal_symbols = ["<:diamond:1136554333723435068>", "<:platinum:1136554336705597510>", "<:gold:1136554335426318406>", "<:silver:1136554338895011850>", "<:bronze:1136554332586786879>"]

exports.level_symbols = ["<:bronze:1136554332586786879>", "<:silver:1136554338895011850>", "<:gold:1136554335426318406>", "<:platinum:1136554336705597510>", "<:diamond:1136554333723435068>", "<:master:1145487065912447026>", "<:grandmaster:1145487067682447442>"]

// XP progress bar segments. Tiles butt together seamlessly, so a bar is just N of these
// in a row: `filled` for progress already earned, `new` for what was just gained, `empty`
// for the rest. The `*_half` tiles are the half-and-half boundary pieces, for bars whose
// progress lands mid-tile.
exports.bar_symbols = {
    filled: "<:Filled:1531757562108837928>",
    new: "<:New:1531757614269206588>",
    empty: "<:Empty:1531757513999909034>",
    // The segment you were attempting and lost, for a bar that tracks attempts rather than
    // pure progress.
    error: "<:Error:1532842628624810077>",
    filled_half: "<:FilledHalf:1531758373194829955>",
    filled_new_half: "<:FilledNewHalf:1531757539673247985>",
    new_half: "<:NewHalf:1531757586435543212>"
}
