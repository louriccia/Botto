...
const { simulateSpeed } = require('./simulate/kbd_simulator.js')
...
module.exports = {
    ...
    execute(client, interaction, args) {
        ...
        var postColumn2 = ""
        var postColumn3 = ""
        var output = simulateSpeed(track, fps, upgrades, laps) //format:: [[pod index, average speed, finish time], [next pod], ... [final pod], fps, track]
        for (let i = 0; i < 23; i++) {
            postColumn1 += racers[output[i][0]].flag + " " + racers[output[i][0]].name + "\n"
            postColumn2 += Math.round(output[i][1] * 10) / 10 + "\n"
            postColumn3 += time_fix(output[i][2]) + "\n"
        }
        myEmbed
